...
public class Production extends JPanel {
    ...
    public void init(){
	  ...
	  dtm1=new DefaultTableModel();
  	  ...
  	  Constants.tableloader("Production",table,scrollpane,dtm);
    ...
    public void selected(){
    	  table.getSelectionModel().addListSelectionListener(new ListSelectionListener(){
  			...
  			public void valueChanged(ListSelectionEvent e) {
              	...
              	String qry="select product.name as productname,Supervisor.name as supervisor,RawMaterials.name as rawmaterial"
						+ " from Production join product on Production.p_id=product.p_id "
						+ "join Supervisor on Production.supervisor_id=Supervisor.supervisor_id"
						+ "	join RawMaterials on Production.rm_id=RawMaterials.rm_id "
						+ "join Suppliers on	RawMaterials.supplier_id=Suppliers.supplier_id where Production.production_id='"+s1+"'";
			...
			textfield12.setText(table.getModel().getValueAt(index, 7).toString());	
    ...
    public void delete(){
     ...
     Connection connection = DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=austproject;integratedSecurity=true");  
      ...
      Statement statement = connection.createStatement();  
       ...
       statement.executeUpdate(" delete from Production where production_id='"+s1+"'");
    ...
    public void insert(){
    	  but2.addActionListener(new ActionListener(){
              public void actionPerformed(ActionEvent e) {
              	...
              	String s7=textfield13.getText().toString();
     ...
     Connection connection = DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=austproject;integratedSecurity=true");  
      ...
      Statement statement = connection.createStatement();  
      ResultSet rs = statement.executeQuery("SELECT  timeperiod FROM product where p_id='"+s2+"' ");  
      ResultSetMetaData  rsmetadata = rs.getMetaData();
      int timeperiod=0;
      while(rs.next()){
       timeperiod =Integer.valueOf(rs.getString(1));
         }  
      String dt = s6;
      ...
      statement.execute(" insert into Production "  + " values ( '"+s1+"', '"+s2+"','"+s3+"',"+s4+","+s5+",'"+s6+"','"+dt+"','"+s7+" ')");
      ResultSet resultSet=null;
//      resultSet.close();
//      statement.close();
   ...
   Connection connection = DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=austproject;integratedSecurity=true");  
    ...
    statement.execute(" update Production set production_id='"+s1+"',p_id='"+s2+"',supervisor_id='"+s3+"',productionquantity="+s4+",productioncost="+s5+",mfgdate='"+s6+"',rm_id='"+s7+"' where production_id='"+s1+"'"); 
    ResultSet resultSet=null;
//    resultSet.close();
//    statement.close();
